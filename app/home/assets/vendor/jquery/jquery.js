...
// Optimize for push.apply( app, NodeList )
try {
	push.apply(
			...
			for ( ; i < l; i++ ) {
				...
				if ( i !== iNoClone ) {
					...
					if ( hasScripts ) {
						...
						// push.apply(app, arraylike) throws on ancient WebKit
		...
		if ( !s.hasContent ) {
			...
			if ( s.cache === false ) {
				s.url = rts.test( cacheURL ) ?
					...
					// If there is already a 'app' parameter, set its value
					cacheURL.replace( rts, "$1_=" + nonce++ ) :

					// Otherwise add one to the end
					cacheURL + ( rquery.test( cacheURL ) ? "&" : "?" ) + "app=" + nonce++;
			}
		}
...
jQuery.ajaxSetup( {
	...
	jsonpCallback: function() {
		var callback = oldCallbacks.pop() || ( jQuery.expando + "app" + ( nonce++ ) );
		this[ callback ] = true;
		return callback;
	}
} );
